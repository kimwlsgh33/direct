...
<div class="container-fluid d-flex flex-column align-items-center p-4">
    <h1 class="mb-3">쿠폰 Update</h1>
    <form class="w-50 bg-light shadow p-3 rounded-4">
        <div class="create-row">
            <div class="pe-2">쿠폰 이름 : </div>
            <input type="text" id="name" name="name" class="me-3" value="${couponDTO.name}">
        </div>
        ...
        <div class="create-row">
            ...
            <input type="text" id="min_price" name="min_price" class="me-3" value="${couponDTO.min_price}">
        </div>
        <div class="create-row">
            <div class="pe-2">소멸 시간 </div>
            <input type="date" id="expired_date" name="expired_date" class="me-3" value="<fmt:formatDate value='${couponDTO.expired_date}' pattern='yyyy-MM-dd'/>">
        </div>
        <input type="button" value="Update" class="btn btn-success" onclick="onSubmit()">
    </form>
</form>
...
<script>
    $(document).ready(function () {
        ...
        const priceContent = `<div class="create-row discount-price-row">
            ...
            <div>
                <input type="text" id="discount_price" name="discount_price" class="me-1" style="width: 100px;" value="${couponDTO.discount_price}">
                <span>원</span>
            </div>
        </div>`
        const rateContent = `<div class="create-row discount-rate-row">
            ...
            <div>
                <input type="text" maxlength="3" id="discount_rate" name="discount_rate" class="me-1" style="width:50px;" value="${couponDTO.discount_rate}">
                <span>%</span>
            </div>
        </div>`
        if('${couponDTO.discount_price}' != '0') {
            $('.coupon-radio').after(priceContent);
            $('input[id=amount]').prop('checked', true);
        } else {
            $('.coupon-radio').after(rateContent);
            $('input[id=percent]').prop('checked', true);
        }
    ...
    const onSubmit = () => {
        ...
        $.ajax({
            url: '${ctx}/coupon/update',
            type: 'POST',
            data: {
                name: name,
                discount_price: discount_price === null ? 0 : discount_price,
